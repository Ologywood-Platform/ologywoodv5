...
import { ExternalLink, ImageIcon, ShoppingBag, ShoppingCart } from 'lucide-react';
...
export function MerchDisplay({ userId, userType, talentType }: MerchDisplayProps) {
  ...
  function openItem(item: any) {
    ...
    if (item.trackInventory && (item.inventoryQuantity ?? 0) <= 0) return;
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        {items.map((item: any) => {
          ...
          return (
            <Card key={item.id} className={`overflow-hidden group transition-shadow ${soldOut ? 'opacity-75' : 'cursor-pointer hover:shadow-md'}`} onClick={() => openItem(item)}>
              <div className="relative aspect-square bg-gray-100 dark:bg-gray-800">
                ...
                {!soldOut && <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-center justify-center"><Button size="sm" className="opacity-0 group-hover:opacity-100 transition-opacity bg-white text-black hover:bg-white/90 gap-1.5 text-xs">{item.sellingMethod === 'ologywood' ? <ShoppingCart className="h-3 w-3" /> : <ExternalLink className="h-3 w-3" />}{item.sellingMethod === 'ologywood' ? 'Order' : 'Visit store'}</Button></div>}
              </div>
      ...
      <MerchCheckoutDialog item={selectedItem} open={!!selectedItem} onOpenChange={(open) => { if (!open) setSelectedItem(null); }} />
    </div>
  );
}
