...
export function SupportChat({ userId, userName, userEmail, onClose }: SupportChatProps) {
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed bottom-4 right-4 z-40 flex items-center justify-center w-14 h-14 bg-purple-600 text-white rounded-full shadow-lg hover:bg-purple-700 transition-colors"
  ...
  return (
    <div className="fixed bottom-0 right-0 left-0 sm:bottom-6 sm:right-6 sm:left-auto z-50 w-screen sm:w-96 max-h-screen sm:max-h-[600px] flex flex-col bg-white rounded-t-3xl sm:rounded-lg shadow-2xl border-0 sm:border border-gray-200 sm:mx-0">
      {/* Header */}
