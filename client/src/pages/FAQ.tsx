...
export default function FAQ() {
  ...
  const faqs = [
    ...
    // Stripe & Webhooks
    { q: 'I got an email from Stripe about a failing webhook. What do I do?', a: 'This happens when a temporary development URL expires. Go to Stripe Dashboard → Developers → Webhooks, delete the old endpoint, and add a new one with your production URL (https://ologywood-mp6flm6c.manus.space/api/stripe/webhook). Copy the new Signing Secret and update it in Settings → Payment. This does not affect existing payments.' },
    { q: 'How do I set up Stripe for production?', a: 'Complete Stripe KYC verification, then enter your live API keys in Settings → Payment. Add a production webhook endpoint in Stripe Dashboard → Developers → Webhooks pointing to https://ologywood-mp6flm6c.manus.space/api/stripe/webhook. Update the webhook signing secret in Settings → Payment. Test with card 4242 4242 4242 4242 before going live.' },
