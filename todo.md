...
## EXTERNAL MERCH ACCESS RESTRICTION (Aug 27, 2026)
- [x] Trace Dawud’s saved external merch URL and the full redirect chain from his profile and product page
- [x] Confirm the temporary restriction was generated by Etsy’s DataDome security after leaving OlogyWood
- [x] Verify the saved destination is Dawud’s canonical public Etsy listing rather than an editor, preview, tracking, or protected link
- [x] Add a clear external-store confirmation and recovery path so fans are not left in a confusing dead end
- [x] Preserve direct OlogyWood checkout behavior for native merch items
...
## NIL PROTECT COLLEGE SPORTS ACT READINESS ASSESSMENT (Sep 15, 2026)
- [x] Verified from Congress.gov, the Senate Commerce Committee’s August revised floor text, CBO, FTC, current U.S. Code, and the user-provided Husch Blackwell summary that S. 4668 remains proposed—not enacted—and mapped its five-percent athlete-agent fee cap, written-contract protections, reporting rules, dispute rights, state-law treatment, and enforcement framework
- [x] Distinguished the proposed athlete-agent compensation cap from OlogyWood’s separately described booking, Ology Live, Fan Club, ticketing, subscription, Stripe-processing, and marketplace-service fees; no blanket fee change was made
- [x] Audited athlete-only guidance, athlete onboarding/profile data, rider templates, NIL contracts, e-signatures, AI contract analysis, institutional-disclosure language, Ology Live agreements/earnings, Terms, and actual fee/payment logic
- [x] Mapped the verified requirements to OlogyWood controls and identified strong foundations, unsupported compliance claims, missing student-athlete/agent intake, incomplete endorsement terms, reporting gaps, fee-classification risk, arbitration exposure, and attorney-review points
- [x] Delivered `docs/nil-protect-college-sports-act-assessment.md` with cited findings and prioritized product, contract, disclosure, fee-separation, reporting, role-gating, and governance recommendations; TypeScript and repository hygiene pass, and no terms, contracts, fees, production code, or production data were changed

## NIL READINESS PROTECTIONS IMPLEMENTATION (Sep 15, 2026)
- [x] User selected in-app deadline reminders rather than automatic daily background reminders; deadlines will be recalculated when authenticated athletes open NIL tools or relevant dashboard surfaces, with no scheduled job or email automation
- [x] Added an additive legacy-safe student-athlete compliance profile, NIL deal classification, representative verification, disclosure evidence, contract versioning, and in-app deadline data model; managed migration `0112_grey_triton.sql` and the idempotent compatibility guard contain no destructive DDL
- [x] Added private Athlete intake for college/prospective/pro/former status, institution, division/association, institution state, eligibility end date, compliance contact, minor/guardian status, representative involvement, and athlete attestation; none of these fields is returned by public-profile procedures
- [x] Added deal-level endorsement classification, representative identity and state registration, separate agent/platform/Stripe fee fields, and a proposed 5% hard stop only for protected student-athlete endorsement workflows; existing 1%, 15%, subscription, and Stripe calculations remain unchanged
- [x] Replaced unsupported “NCAA Compliant” claims across Terms, contracts, Help, FAQ, onboarding, Contract Analyzer/Viewer, Ology Live summaries, and OlogyWood AI with athlete self-certification plus institutional and qualified legal review language; false platform-filing and escrow wording was removed
- [x] Added proposed written-contract protections for parties, services, compensation, nonperformance termination, enrollment/residency nonconditioning, eligibility-limited term, representative details, fee separation, Terms/template versions, and limited contract-safe compliance snapshots
- [x] Added a student-athlete non-waivable statutory-rights carve-out to Ology Live and generated NIL contract dispute language while preserving ordinary platform dispute handling for unrelated agreements
- [x] Implemented $600 same-source/rolling-12-month aggregation, five-day agreement and thirty-day compensation in-app deadlines, derived statuses, athlete/representative attestations, optional private S3 evidence, owner-authorized short-lived retrieval, storage-key redaction, and append-only audit history without claiming OlogyWood files disclosures
- [x] Restricted NIL templates, NIL reports, contract analysis, and private compliance controls to authenticated owned Athlete profiles server-side while keeping Ology Live creation and general earnings available to every creator type
- [x] Updated versioned Terms and the in-app notice, booking/Ology Live fee labels and Stripe metadata, Help, FAQ, onboarding tour, Ology Live summaries, deterministic OlogyWood AI guidance, and NIL contract analysis for the non-agent model, proposed 5% scope, reporting workflow, and legal-review limits
- [x] Validated Athlete and non-Athlete authorization, representative fee rejection/acceptance, legacy schema repair, rolling reminders, submission attestations, private event output, zero-residue cleanup, responsive mobile/desktop layout, ordinary creator Ology Live access, payments, signatures, and unchanged real NIL/payment records
- [x] TypeScript passed; 535 focused NIL/security/payment/contract tests passed; the complete suite passed with 2,809 tests and 23 skipped; production build and repository hygiene passed with no temporary artifacts or exposed secrets
- [x] Saved publishable implementation checkpoint `ab7198f4`; qualified sports-law counsel must still approve the Terms amendment, generated NIL contract clauses, proposed 5% covered-endorsement scope, student-athlete statutory-rights carve-out, and institutional/state disclosure workflow before OlogyWood relies on them as legal compliance controls
- [x] Removed the final live homepage “NCAA Compliant” certification claim discovered during post-publication verification and replaced it with “NIL Readiness Tools,” private records/reminders/fee-separation language, and an explicit institutional-or-legal-review boundary
- [x] Re-audited production source with no affirmative certification claim remaining, added a homepage no-regression assertion, passed 86 affected tests, TypeScript, the complete 2,809-test suite with 23 skipped, and production build; published corrected checkpoint `7299874d` and verified the live homepage shows “NIL Readiness Tools,” the amended Terms and Help guidance are live, and signed-out `/nil-compliance` access redirects without exposing private data

## UNIFIED DISCOVERY NAVIGATION & SEARCH (Sep 17, 2026)
- [x] Added dynamic, responsive breadcrumb trails to public artist, venue, and event detail pages with aligned JSON-LD hierarchy and stable destination links
- [x] Added a shared header search dialog on desktop and mobile for public Talent, Venues, and Events, with 300 ms auto-suggest, grouped results, public images, contextual metadata, clean canonical links, empty/error/loading states, and Ctrl/Cmd+K support
- [x] Preserved strict one- and two-character title-prefix matching so a single letter does not return unrelated records; longer queries can also match names, titles, cities, states, and locations
