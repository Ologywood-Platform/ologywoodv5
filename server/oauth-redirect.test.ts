...
describe("OAuth Redirect Configuration", () => {
  it("should have correct OAuth redirect base URL configured", () => {
    ...
    // In dev environment, should use manus.computer domain (temporary deployment)
    // In production, should use manus.space domain
    ...
    expect(oauthRedirectBase).toMatch(/manus\.(computer|space)/);
    expect(oauthRedirectBase).toContain("https://");
  });
